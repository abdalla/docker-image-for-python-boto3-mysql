#!/bin/bash
# Microsoft ODBC Driver 13 for SQL Server Installer
...
# Strings listed here
driver_name="Microsoft ODBC Driver 13 for SQL Server";
driver_version="13.0.0.0"
driver_dm_name="ODBC Driver 13 for SQL Server"
...
req_libs=( libc6 libkrb5-3 libgss3 e2fsprogs openssl )

#language of the install
lang_id="en_US";

# files to be copied by directory
driver_file="libmsodbcsql-13.0.so.0.0"

lib_files=( "lib64/$driver_file" )
lib_perms=( 0755 )

bin_files=( "bin/bcp-13.0.0.0" 
            "bin/sqlcmd-13.0.0.0" )
bin_sym=( bcp sqlcmd )
bin_perms=( 0755 0755 )

sup_files=( install.sh build_dm.sh README LICENSE WARNING INSTALL)
sup_perms=( 0755 0755 0644 0644 0644 0644 )

rll_files=( "bin/bcp.rll" 
            "bin/SQLCMD.rll"
            "bin/BatchParserGrammar.dfa"
            "bin/BatchParserGrammar.llr" 
            "lib64/msodbcsqlr13.rll" )
...
function check_required_libs
{
    ...
    for lib in ${req_libs[@]}
    do
        local present=$(dpkg -l $lib 2> /dev/null | grep '^ii') >> $log_file 2>&1
        if [ "$present" == "" ]; then
            log "The $lib library was not found installed in the package database."
